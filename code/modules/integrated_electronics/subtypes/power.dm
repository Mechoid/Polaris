...
// External battery to assembly.

/obj/item/integrated_circuit/power/siphon
	name = "power siphon circuit"
	desc = "This can wirelessly siphon electricity from an external machine to the assembly's battery."
	icon_state = "power_transmitter"
	extended_desc = "This circuit transmits 5 kJ of electricity every time the activator pin is pulsed. The input pin must be \
	a reference to a machine to pull electricity from.  This can be a battery, or anything containing a battery.  The machine can exist \
	inside the assembly, or adjacent to it.  If the target is outside of the assembly, \
	some power is lost due to ineffiency."
	w_class = ITEMSIZE_SMALL
	complexity = 16
	inputs = list("target" = IC_PINTYPE_REF)
...
/obj/item/integrated_circuit/power/siphon/large
	name = "large power siphon circuit"
	desc = "This can wirelessly siphon a lot of electricity from an external machine to the assembly's battery.  Warning:  Do not operate in flammable enviroments."
	extended_desc = "This circuit transmits 20 kJ of electricity every time the activator pin is pulsed. The input pin must be \
	a reference to a machine to pull electricity from.  This can be a battery, or anything containing a battery.  The machine can exist \
	inside the assembly, or adjacent to it.  If the target is outside of the assembly, \
	some power is lost due to ineffiency."
...
/obj/item/integrated_circuit/power/siphon/do_work()

	var/atom/movable/AM = get_pin_data_as_type(IC_INPUT, 1, /atom/movable)
	if(AM)
		if(!assembly)
			return FALSE // Pointless to do everything else if there's no battery to charge.
		...
		if(cell)
			...
			if(A.Adjacent(B))
				...
				if(transfer_amount && cell.checked_use(amount_to_move))
					assembly.give_power(transfer_amount) // CELLRATE is already handled in give_power()
				AM.update_icon()
...
/obj/item/integrated_circuit/power/siphon/large/do_work()
	if(..()) // If the above code succeeds, do this below.
		if(prob(2))
			var/datum/effect/effect/system/spark_spread/sparks = new /datum/effect/effect/system/spark_spread()
			sparks.set_up(3, 0, get_turf(src))
